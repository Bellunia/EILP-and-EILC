package other_algorithms.negative_statements;
...
//input files is the a tabular dataset (.tsv)
        }
        ...
        private static void inferUsefulNegations(String input, int peering_col, int k) throws IOException {
            ...
            if ((line = br.readLine()) != null)
            {
                line=line.trim();
                features=line.split("\t");
            }// the first line presents the feature name
        ...
        private static void writetopk(String e, HashMap<String, Double> candidates, int k, BufferedWriter wr, Set<String> peers) throws IOException {
            ...
            for(int i=1; i<=k; i++)
            {
                ...
                try { key=Collections.max(candidates.entrySet(), Entry.comparingByValue()).getKey(); }
