...
import gilp.query.QueryEngine;
import gilp.query.SimpleCNFQueryEngine;
import gilp.rdf3x.*;
...
public class RulePackage {
	...
	//	if (!cls.getIterator().hasNext()) {
		...
		HashSet<Triple> covered_triples = sg_set.getAllTriples();
	...
	void calcSupportInKB() {
		...
		RDFPredicate first_tp =  cls.getIterator().get(0);
	...
	public RDFSubGraphSet getSubgraphsCoveredByRule(){	
		...
		for (RDFSubGraph sg: sg_set.getSubGraphs()){
			...
			int i=0;
			for(RDFPredicate tp :sg_set.getPredicates()){
