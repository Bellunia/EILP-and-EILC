...
import gilp.knowledgeClean.RDFBFSLearner;
...
public class RuleBaseManager {
	...
	static RDFRuleImpl appySubstituions(RDFRuleImpl r, HashMap<String, String> hmapSubstitutions){
		String newVar = null;
		ArrayList<RDFPredicate> preds = r.get_body().getIterator();
	...
	public static boolean isConflict(RDFRuleImpl r1, RDFRuleImpl r2){
		...
		ArrayList<RDFPredicate> preds = r2Copy.get_body().getIterator();

		for (RDFPredicate pred : preds) cls.addPredicate(pred);
