...
public class RDFRuleImpl extends Rule {
	...
	public void removeSubsumedPredicates(){
		ArrayList<RDFPredicate> iterPred = this._body.getIterator();
		ArrayList<RDFPredicate> iterPred2 = this._body.getIterator();
		ArrayList<RDFPredicate> listPredicates = new ArrayList<>(iterPred);
		
		ArrayList<RDFPredicate> subsumedPredicates = new ArrayList<>();
		for (int i=0;i<listPredicates.size();i++){
			RDFPredicate tp1 =listPredicates.get(i);
			for (int j=i+1;j<listPredicates.size();j++){
				RDFPredicate tp2 = listPredicates.get(j);
		...
		for (RDFPredicate p: subsumedPredicates){
	...
	public void normalize(){
		...
		RDFPredicate[] predicates = new RDFPredicate[this.get_body().getBodyLength()];
		int i = 0;
		for (RDFPredicate rdfPredicate : myIter) predicates[i++] = rdfPredicate;
	...
	public ArrayList<String> findFreshArgument(RDFPredicate tp){
		...
		ArrayList<RDFPredicate> iterPred = this.get_body().getIterator();
		
		boolean freshSub = true;
		boolean freshObj = true;

		for (RDFPredicate p : iterPred) {
	...
	public boolean isSafe(){
		...
		ArrayList<RDFPredicate> iterPreds = this.get_body().getIterator();
		for (RDFPredicate tp : iterPreds) {
